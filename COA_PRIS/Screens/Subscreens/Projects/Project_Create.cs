...
namespace COA_PRIS.Screens.Subscreens.Projects
{
    public partial class Project_Create : Form
    {
        ...
        public void InitzializeControls()
        {
            ...
            PRIS_Status = new PRIS_Label_Status(_title: "Status :", _options: ret, _readOnly: true, _value: "Pending");
            PRIS_Status.Dock = DockStyle.Fill;
        ...
        private List<UserControl[]> PRISUserControls() 
        {
            ...
            List<UserControl[]> control = new List<UserControl[]>
            {
                new UserControl[]
                {
                    ...
                    new PRIS_Label_Rich(_title: "Project Title :", _isRequired: true, _entryHeight: 270),
                ...
                new UserControl[]
                {
                    //new PRIS_Label_Rich(_title: "Subject : ", _isRequired: true, _entryHeight: 385),
                ...
                new UserControl[] 
                {
                    ...
                    new PRIS_Label_Rich(_title: "Remarks : ", _isRequired: false, _entryHeight: 280),
                }
        ...
        private void create_Btn_Click(object sender, EventArgs e)
        {
            ...
            using (database_Manager)
            {
                ret += database_Manager.ExecuteNonQuery(string.Format(Database_Query.set_new_docu_infos, project_id.Text, values["Project Number"], values["Project Title"], values["Amount"], values["Period"], values["Subject"], values["Date Received"]));

                ret += database_Manager.ExecuteNonQuery(string.Format(Database_Query.set_new_history, history_code, string.Format(Log_Message.set_docu_message, project_id.Text, values["Project Number"], values["Project Title"], Activity_Manager.CurrentUser), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), project_id.Text));

                ret += database_Manager.ExecuteNonQuery(string.Format(Database_Query.set_new_remarks, remarks_code, values["Remarks"], project_id.Text));

                ret += database_Manager.ExecuteNonQuery(string.Format(Database_Query.set_new_docu_trans, project_id.Text, values["Agency"], values["Contractor"], values["Nature"], values["Division"], values["TAS"], remarks_code, PRIS_Status.Value, null));
            }

            if (ret == 4) 
            {
                MessageBox.Show($"{values["Project Number"]} - {values["Project Title"]} is successfully added.", "PRIS Record Confirm", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ResetControls();
        ...
        private void ResetControls() 
        {
            ...
            validator.PRISClearErrors(control_Panel, error_Provider, error_Message);
        }
